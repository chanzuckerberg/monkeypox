...
def register_arguments(parser):
    parser.add_argument(
        "--scheme", required=True, metavar="YAML", help="subsampling scheme"
    )
    parser.add_argument(
        "--output-dir",
        required=True,
    ...
    parser.add_argument(
        "--alignment", required=True, metavar="FASTA", help="alignment to subsample"
    )
    parser.add_argument(
        "--alignment-index",
        required=False,
        metavar="INDEX",
        help="sequence index of alignment",
    )
    parser.add_argument(
        "--reference",
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="TXT",
        help="strains to force exclude",
    )
    parser.add_argument(
        "--output-fasta",
        required=True,
        metavar="FASTA",
        help="output subsampled sequences",
    )
    parser.add_argument(
        "--output-metadata",
        required=True,
        metavar="TSV",
        help="output subsampled metadata",
    )
    parser.add_argument(
        "--output-log",
        required=False,
        metavar="TSV",
        help="log file explaining why strains were excluded / included",
    )
    parser.add_argument(
        "--use-existing-outputs",
        required=False,
        action="store_true",
        help="use intermediate files, if they exist",
    )
...
def run(args):
    ...
    traverse_graph(graph, lambda s: s.filter())
...
def validate_scheme(scheme):
    try:
        ...
        with open(
            path.join(path.dirname(path.realpath(__file__)), "subsample_schema.yaml")
        ) as fh:
            schema = yaml.safe_load(fh)
    except yaml.YAMLError as err:
        raise Exception(
            "Subsampling schema definition is not valid YAML. Error: {}".format(err)
        )
    ...
    try:
        ...
        raise Exception(
            "Subsampling schema definition is not valid. Error: {}".format(path, err)
        )
...
class Sample:
    ...
    def initialise_filter_args(self, config, subsample_args):
        ...
        arg_list = [
            ...
            "--output",
            path.join(
                self.tmp_dir, f"sample.{self.name}.fasta"
            ),  # filtered sequences in FASTA forma
            "--output-metadata",
            path.join(
                self.tmp_dir, f"sample.{self.name}.tsv"
            ),  # metadata for strains that passed filters
            "--output-strains",
            path.join(
                self.tmp_dir, f"sample.{self.name}.txt"
        ...
        if unused_args:
            print(
                f"Warning - the following config parameters are not part of augur filter and may be ignored:"
            )
    ...
    def calculate_required_priorities(self):
        ...
        if not focal_sample:
            raise Exception(
                f"Cannot calculate priorities needed for {self.name} as the {self.get_priority_focus_name()} sample wasn't linked"
            )
        print(
            f"Calculating priorities of {focal_sample.name}, as required by {self.name}"
        )
    ...
    def calculate_priorities(self):
        ...
        if self.use_existing_outputs and check_outputs_exist(proximity_output_file):
            ...
            get_distance_to_focal_set(
                ...
                ignore_seqs=[
                    "Wuhan/Hu-1/2019"
                ],  # TODO - use the config to define this?
            )
    ...
    def filter(self):
        print(
            "\n---------------------------------\nCONSTRUCTING SAMPLE FOR",
            self.name,
            "\n---------------------------------",
        )
        self.calculate_required_priorities()
        if self.use_existing_outputs and check_outputs_exist(
            self.filter_args.output_metadata,
            self.filter_args.output_strains,
        ...
        try:
            self.sampled_strains = set(
                pd.read_csv(self.filter_args.output_strains, header=None)[0]
            )
        except pd.errors.EmptyDataError:
            self.sampled_strains = set()
        self.filter_log = pd.read_csv(
            self.filter_args.output_log, header=0, sep="\t", index_col="strain"
        )
...
def combine_samples(args, samples):
    ...
    print(
        f"In total, {len(sampled_strains)} strains are included in the resulting subsampled dataset"
    )
    ...
    for metadata in metadata_reader:
        ...
        metadata_mode = "a"
    print(
        f"{metadata_written_to_disk} metadata entries written to {args.output_metadata}"
    )
